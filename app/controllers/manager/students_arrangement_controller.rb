class Manager::StudentsArrangementController < ManagerMainController
  ...
  def add_student_to_room
    ...
    if room_id != @room.id
      if @room.number_student < @room.max_number_student 
        ...
        respond_to do |format|
          format.json {render json:{message: "Xin lỗi! Thêm sinh viên vào phòng thất bại :(", room_max_number_student: @room.max_number_student, room_student_number: @room.number_student, room: @room, room_id: room_id},  status: :bad_request}
        end
      end
    end
  end
  ...
  def search_rooms_arrangement
    if params[:q] 
      @rooms = Room.where.not(id: 1).ransack(room_name_or_room_type_cont: params[:q]).result.order('order_name ASC')
      if @rooms
        respond_to do |format|
